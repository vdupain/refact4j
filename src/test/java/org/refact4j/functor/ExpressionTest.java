...
public class ExpressionTest {
	...
	public void testExpressions() {
		...
		boolean flag1 = fc.eval(dummyEntity);
		boolean flag2 = expression.eval(dummyEntity);
		boolean flag3 = beanExpression.eval(dummy);
		assertTrue(flag1);
		assertEquals(flag1, flag2);
		assertEquals(flag2, flag3);

		expression = EntityExpressionBuilder.init(FooDesc.VALUE).greaterThan(10.).and(
				(EntityExpressionBuilder.init(FooDesc.NAME).equalTo(new ConstantUnaryFunctor<String>("azerty"))))
				.getExpression();
		...
		UnaryFunctor<EntityObject, Boolean> func3 = new BinaryCompose(new And(), fc1, fc2);
		...
		flag1 = func3.eval(dummyEntity);
		flag2 = expression.eval(dummyEntity);
		flag3 = beanExpression.eval(dummy);
		assertEquals(flag1, flag2);
		assertEquals(flag2, flag3);
		...
		System.out.println(">>" + expression);
		assertTrue(expression.eval(dummyEntity));
		assertTrue(beanExpression.eval(dummy));
		...
		beanExpression = BeanExpressionBuilder.init().not(BeanExpressionBuilder.init("Value").greaterThan(10.)).and(
				...
				.getExpression();
		assertTrue(otherExpression.eval(dummyEntity));
		assertTrue(beanExpression.eval(dummy));
	...
	public void testExpressionsWithFunctors() {
		UnaryFunctor<EntityObject, Double> getValue = new UnaryFunctor<EntityObject, Double>() {
			public Double eval(EntityObject obj) {
		...
		EntityObject dummyEntity = EntityObjectBuilder.init(FooDesc.INSTANCE).setFieldValue(FooDesc.VALUE, 12.)
				.getEntity();
		Boolean o1 = fc.eval(dummyEntity);
		Boolean o2 = expression.eval(dummyEntity);
		assertTrue(o1);
		assertEquals(o1, o2);
	...
	public void testSimpleExpression() {
		...
		assertFalse(exprGreaterOrEqual100.eval(99.));
		assertTrue(exprGreaterOrEqual100.eval(101.));
	}
	...
	public void testLikeExpression() {
		...
		assertTrue(exprLike.eval("abcde"));
	}
	...
	public void testBetweenExpression() {
		...
		assertTrue(expression.eval(dummyEntity8));
		EntityObject dummyEntity12 = EntityObjectBuilder.init(FooDesc.INSTANCE).setFieldValue(FooDesc.VALUE, 12.)
				.getEntity();
		assertFalse(expression.eval(dummyEntity12));

		Expression betweenExpr = EntityExpressionBuilder.init(FooDesc.VALUE).between(0., 10.).getExpression();
		assertTrue(betweenExpr.eval(dummyEntity8));
		assertFalse(betweenExpr.eval(dummyEntity12));
		printEquivalentExpr(expression, betweenExpr);

		Between between = new Between(0., 10.);
		assertTrue(between.eval(dummyEntity8.get(FooDesc.VALUE)));
		assertFalse(between.eval(dummyEntity12.get(FooDesc.VALUE)));
		...
		assertTrue(between.eval(dummy8.getValue()));
		assertFalse(between.eval(dummy12.getValue()));
	}
	...
	public void testIsNullAndIsNotNullExpressions() {
		...
		assertTrue(expression.eval(dummyEntityNull));
		EntityObject dummyEntity12 = EntityObjectBuilder.init(FooDesc.INSTANCE).setFieldValue(FooDesc.VALUE, 12.)
				.getEntity();
		assertFalse(expression.eval(dummyEntity12));
		Expression isNullExpression = EntityExpressionBuilder.init(FooDesc.VALUE).isNull().getExpression();
		assertTrue(isNullExpression.eval(dummyEntityNull));
		assertFalse(isNullExpression.eval(dummyEntity12));
		...
		assertFalse(expression.eval(dummyEntityNull));
		assertTrue(expression.eval(dummyEntity12));
		Expression isNotNullExpression = EntityExpressionBuilder.init(FooDesc.VALUE).isNotNull().getExpression();
		assertFalse(isNotNullExpression.eval(dummyEntityNull));
		assertTrue(isNotNullExpression.eval(dummyEntity12));
	...
	public void testInAndNotInExpressions() {
		...
		assertTrue(expression.eval(dummyEntity1));
		assertFalse(expression.eval(dummyEntity3));

		Object[] values = new Object[] { 0., 1., 2. };
		Expression inExpression = EntityExpressionBuilder.init(FooDesc.VALUE).in(values).getExpression();
		assertTrue(inExpression.eval(dummyEntity1));
		assertFalse(inExpression.eval(dummyEntity3));
		...
		assertTrue(inExpression2.eval(dummyEntity1));
		assertFalse(inExpression2.eval(dummyEntity3));
		...
		expression = EntityExpressionBuilder.init().not(
				EntityExpressionBuilder.init(FooDesc.VALUE).equalTo(0.).or(
						EntityExpressionBuilder.init(FooDesc.VALUE).equalTo(1.)).or(
						EntityExpressionBuilder.init(FooDesc.VALUE).equalTo(2.))).getExpression();
		assertTrue(expression.eval(dummyEntity3));
		assertFalse(expression.eval(dummyEntity1));

		Expression notInExpression = EntityExpressionBuilder.init(FooDesc.VALUE).notIn(values).getExpression();
		assertTrue(notInExpression.eval(dummyEntity3));
		assertFalse(notInExpression.eval(dummyEntity1));
	...
	public void testMaxLength() {
		...
		assertTrue(expression.eval("abc"));
		assertFalse(expression.eval("abcdef"));
		assertTrue(expression.eval(null));
	}

	public static void printEquivalentExpr(UnaryFunctor functor1, UnaryFunctor functor2) {
